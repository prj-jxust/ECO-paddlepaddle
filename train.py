...
from model import ECO_model
from reader import KineticsReader
from configs import parse_config, merge_configs, print_configs
...
def train(args):
    ...
    with fluid.dygraph.guard(place):
        ...
        train_model = ECO_model.ECOModel()
        opt = fluid.optimizer.Momentum(0.001, 0.9, parameter_list=train_model.parameters())
        #opt = fluid.optimizer.AdamOptimizer(0.001, parameter_list=train_model.parameters())

        if args.pretrain:
            # 加载上一次训练的模型，继续训练
            model, _ = fluid.dygraph.load_dygraph('work/'+args.save_dir + '/ECO_model1')
        ...
        for i in range(epochs):
            for batch_id, data in enumerate(train_reader()):
                ...
                if batch_id % 50 == 0:
                    ...
                    fluid.dygraph.save_dygraph(train_model.state_dict(), 'work/'+args.save_dir + '/ECO_model1')
